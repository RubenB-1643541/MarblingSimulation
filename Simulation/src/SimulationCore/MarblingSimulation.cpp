...
void MarblingSimulation::BeforeUpdate()
{
	...
	_prevtime = std::clock();
	//Copy freq to freq2
	static_cast<FluidLib::DoubleGrid<IInk>*>(_grids.GetGrid("Ink"))->CopyFirstToSecond();
	static_cast<FluidLib::DoubleGrid<IVelocity>*>(_grids.GetGrid("Vel"))->CopyFirstToSecond();
...
void MarblingSimulation::AfterUpdate()
{
	//Copy freq2 to freq
	//static_cast<FluidLib::DoubleGrid<IInk>*>(_grids.GetGrid("Ink"))->CopyFirstToSecond();
	//static_cast<FluidLib::DoubleGrid<IInk>*>(_grids.GetGrid("Vel"))->CopyFirstToSecond();
	//static_cast<FluidLib::DoubleGrid<IInk>*>(_grids.GetGrid("Ink"))->CopySecondToFirst();
	//static_cast<FluidLib::DoubleGrid<IInk>*>(_grids.GetGrid("Vel"))->CopySecondToFirst();
	...
	if (!_settings.fast) {
		...
		//glCopyNamedBufferSubData(2, 3, NULL, NULL, size);
		//glCopyNamedBufferSubData(src, des, NULL, NULL, size);
...
void MarblingSimulation::CreateBasicTool()
{
	...
	FluidLib::PasteAction<IInk>* hardpastefreq = new FluidLib::PasteAction<IInk>(static_cast<FluidLib::Grid<IInk>*>(_grids.GetGrid("Ink")), FluidLib::ACTION_OPERATION::SET);
	...
	sel->SetHardPasteAction(hardpastefreq);
	...
	hardpastefreq->SetPos(rect->GetXPtr(), rect->GetYPtr());
...
void MarblingSimulation::InitBasicToolComponent(ToolSelectComponent* comp)
{
	...
	comp->AddButton(Button("res/icons/Dripping.png", "Dripping", TOOL_PART::TOOL));
	comp->AddButton(Button("res/icons/Feather.png", "Fan", TOOL_PART::TOOL));
	...
	comp->AddButton(Button("res/icons/Feather.png", "Fan", TOOL_PART::SURFACE));
	...
	comp->AddButton(Button("res/icons/Copy.png", "Copy", TOOL_PART::SELECT_ACTION));
	comp->AddButton(Button("res/icons/Paste.png", "Paste", TOOL_PART::SELECT_ACTION));
	comp->AddButton(Button("res/icons/Paste.png", "HardPaste", TOOL_PART::SELECT_ACTION));
	comp->AddButton(Button("res/icons/Cut.png", "Cut", TOOL_PART::SELECT_ACTION));
